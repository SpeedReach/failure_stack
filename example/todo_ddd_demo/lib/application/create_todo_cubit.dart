...
class CreateTodoCubit extends Cubit<CreateTodoForm> {
  ...
  CreateTodoCubit(this.repository)
      : super(const CreateTodoForm(title: "", description: ""));
  ...
  Future<Result<String, CreateTodoFailure>> createTodo() async {
    ...
    return asyncResultHandleEnvironment(() async {
      ...
      if (missingFields.isNotEmpty)
        return Fail(FormNotCompleteFailure(missingFields));
      ...
      final todoId =
          result.mapFail((failure) => TodoRepositoryFailure(failure)).unwrap();
